...
const [songParams, setSongParams] = createStore({
  title: null, genre: null, year: null, artists: null, album: null
});
...
export function Autofill(props) {
  ...
  onMount(() => {
    if (songParams.artists == null || songParams.artists == "") {
      setErrors({errors: ["You need to fill in artists before auto filling"]})
      return
    }
    path = "/artist"
  ...
  function handleSearch(event) {
    let fuseOptions = { 
      includeScore: true, isCaseSensitive: false,
      shouldSort: true, keys: ['title']
    }
  ...
  function handleSelectResult(event) {
    setSongParams(produce(
      (song) => (song.title = event.target.innerHTML)
    ))
    songSearch.value = event.target.innerHTML
    setSearchResults([])
    ...
    getMusicBrainz(path, params).then(() => {
      ...
      setAlbumSearchResults(resultState.data.data.releases.map((rel) => {
        return {item: rel}
      }));
  ...
  function handleSelectAlbum(event) {
    setSongParams(produce((song) => (song.album = event.target.innerHTML )))
  ...
  <div class="field">
    ...
    <div class="control">
      <input class="input" type="text" ref={songSearch} placeholder="start typing to find song ..." onInput={handleSearch}/>
    </div>
  </div>
  <div class="panel">
    <For each={searchResults()}>
      {(el) => (
      <div class="panel-block has-text-white" value={el.item.id} onClick={handleSelectResult}>
        {el.item.title}
      </div>
      )}
    </For>
  ...
  <div class="field">
    ...
    <div class="control">
      <input class="input" type="text" placeholder="start typing to find song ..." onInput={handleAlbumSearch}/>
    </div>
  </div>
  <div class="panel">
    <For each={albumSearchResults()}>
      {(el) => (
      <div class="panel-block has-text-white" value={el.item.date.slice(0,4)} onClick={handleSelectAlbum}>
        {el.item.title}
      </div>
      )}
    </For>
